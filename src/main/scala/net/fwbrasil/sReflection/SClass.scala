...
object SClass {
    val cache = new HashMap[Class[_], Option[SClass[_]]]() with SynchronizedMap[Class[_], Option[SClass[_]]]
    def find[C](clazz: Class[C]) = cache.getOrElseUpdate(clazz, findClass(clazz).map(new SClass(clazz, _))).asInstanceOf[Option[SClass[C]]]
    def apply[C](clazz: Class[C]) = find[C](clazz).getOrElse(throw new IllegalStateException("Cant find the scala signature."))
}